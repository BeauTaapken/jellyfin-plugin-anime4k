...
export function show(options: Options) {
  ...
  const dialogHelper = globalThis.Dashboard.dialogHelper;
  ...
  if (layoutManager.tv) {
    html += `<button is="paper-icon-button-light" class="btnCloseActionSheet hide-mouse-idle-tv" tabindex="-1" title="Back">
  ...
  if (options.showCancel) {
    ...
    html += `<button is="emby-button" type="button" class="btnCloseActionSheet">Cancel</button>`;
    html += '</div>';
  }
  html += '</div>';
